...
class ImageAnalyzer:
    ...
    def load_memory_and_encode(self, state: ImageAnalyzerState) -> ImageAnalyzerState:
        """Node: Load conversation history and encode image."""
        print(f"\n[1/8] 📚🖼️  Loading memory and encoding image...")
        ...
        try:
            ...
            state["error"] = None
            print(f"   ✓ Image encoded successfully ({len(image_data)} bytes)")
        except FileNotFoundError:
            error_msg = f"Image file not found: {state['image_path']}"
...
or annotated regions drawn on it? Respond ONLY with valid JSON:
{
  ...
  "boxes": [{"id": "box1", "color": "red", "location": "top-left", "label": "text label or null", "contains": "brief description"}]
    ...
    def understand_and_decide_search(self, state: ImageAnalyzerState) -> ImageAnalyzerState:
        ...
        try:
            prompt = f"""You are a query analyzer. Based on the user query "{state['query']}", image context, and conversation history, decide if a web search is needed.
Respond with JSON only: {{ "should_search": true/false, "search_queries": ["query1", "query2"] }}"""
            ...
            if decision.get("should_search"):
                print(f"   ✓ Search NEEDED. Queries: {state['search_queries']}")
            else:
                print(f"   ✓ Search NOT needed.")
        except Exception as e:
